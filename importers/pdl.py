...
class PudloImporter(wg2.importers.base.Importer):
    ...
    def acquire_list(self):
        ...
        ws_data = {
            'action': 'plusetablissement',
            'req[PAYS]': 'France',
            'req[type_poi]': 'Restaurants',
            'req[type_poi_2]': '0',
            'paged': 0
        }
    ...
    def parse_page(self, url, page):
        ...
        ret['ariane_tags'] = json.dumps(
            selector.css('#ariane-line>span>a::attr(title)').getall())
        ret['review_date'] = self.format_date(
            selector.css('span.date_article_time::text').get())
        ...
        street = ''
        city = ''
        for u in selector.css(
                'span.Etab_adresse_mip_ligne>a::attr(href)').getall():
            if re.match('^http://.*', u):
                ret['official_url'] = u
    ...
    def parse_coords(self, page):
        ...
        if gmapurl is not None:
            q = urllib.parse.parse_qs(
                urllib.parse.urlparse(gmapurl)[4])['q'][0]
            if re.match(r'^[\d\.]*,[\d\.\-]*$', q):
                coords_txt = q.split(',')
    ...
    def format_date(self, datetext):
        ...
        result = ''
        date_array = []
        res = re.search(r"\d\d?.*\d\d\d\d", datetext)
        if res:
        ...
        if len(date_array) == 3:
            ...
            if len(j) == 1:
                j = '0' + j
            result = date_array[2] + '-' + self._month_dic.get(
                date_array[1]) + '-' + j
        return result
